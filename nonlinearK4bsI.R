...
scenario.gam.list <- function(scenario, data = NA, explanatory.var = NA, response.var = NA, big.k = 10) {
  ...
  pred.gam.bs <- with(subdata, predict(a.gam.bs, type = "response"))
  ...
  pred2.gam.bs <- predict(a.gam.bs, pred2.points, type = "response", se = T)
  ...
  ssq.gam.bs <- sum((pred.gam.bs - subdata$response)^2)
  ...
  anova.gam.bs <- anova(a.gam.bs, a.gam, test = "Chisq")
...
#  print(anova.gam.k)
  ...
  diff.gam.bs <- sum((pred2.gam.bs$fit - pred2.gam$fit)^2)
  ...
  list(scenario = scenario, aic.gam = aic.gam, bic.gam = bic.gam, ssq.gam = ssq.gam,
       ...
       aic.gam.bs = aic.gam.bs, bic.gam.bs = bic.gam.bs, ssq.gam.bs = ssq.gam.bs,
       edf.gam.bs = sum(a.gam.bs$edf), smooth.gam.bs.P = s.gam.bs$s.pv,
       score.gam.bs = a.gam.bs$gcv.ubre, dev.expl.gam.bs = s.gam.bs$dev.expl,
       aic.lm = aic.lm, bic.lm = bic.lm, ssq.lm = ssq.lm,
       diff.gam.k = diff.gam.k, diff.gam.ml = diff.gam.ml, diff.gam.klm = diff.gam.klm,
       diff.gam.bs = diff.gam.bs, diff.lm = diff.lm,
       anova.gam.k.P = anova.gam.k$P[2], anova.gam.ml.P = anova.gam.ml$P[2],
       anova.gam.klm.P = anova.gam.klm$P[2], anova.gam.bs.P = anova.gam.bs$P[2],
       anova.lm.P = anova.lm$P[2])
}
